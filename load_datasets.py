...
class LoadPixelShiftData(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if 'raw' in self.in_type:
            # linrgb_ = self.blur_layer(linrgb.unsqueeze(0)).squeeze(0)
...
class LoadPreprocessPNG(data.Dataset):
    """
    load training simulated datasets
    """
    ...
    def __init__(self,
        ...
        super(LoadPreprocessPNG, self).__init__()
        self.phase = phase
        self.scale = scale
        self.patch_size = patch_size
        self.downsampler = downsampler
    ...
    def __getitem__(self, index):
        ...
        if self.phase == 'train':
            ...
            srgb = self.test_transforms(srgb)
        # import cv2
        # c = np.transpose(srgb.numpy(),(1,2,0)) * 255
        # c = c.astype('uint8')
        # cv2.imwrite('base.png', c)
        data = {'srgb': srgb}
        ...
        # if noisy is activated, we also need unprocess the rgb (since we only know the shot and read noise of the Raw).
        if 'raw' in self.in_type or 'lin' in self.in_type:
            inv_ccm = unprocess.get_reverse_ccm()
            rggb,linrgb = unprocess.reverse_rgb(srgb, inv_ccm)

            # rgb = np.transpose(linrgb.numpy(),(1,2,0)) * 255
            # rgb = rgb.astype('uint8')
            # cv2.imwrite('adsad0.png',rgb)

            data.update({'raw': rggb.clone(), 'linrgb': linrgb.clone()})
...
class LoadPreprocessBMP_RAW(data.Dataset):
    """
    load training simulated datasets
    """
    ...
    def __init__(self,
        ...
        super(LoadPreprocessBMP_RAW, self).__init__()
        self.phase = phase
        self.scale = scale
        self.patch_size = patch_size
        self.downsampler = downsampler
        ...
        self.patch_size = patch_size
        # read image list from txt
        self.mat_lists = sorted(glob.glob(os.getcwd() + '/' + data_list + "/*" + 'mat'))

    def __len__(self):
        return len(self.mat_lists)

    def __getitem__(self, index):
        # read images
        srgb = loadmat(self.mat_lists[index])['gt']
        rggb = loadmat(self.mat_lists[index])["raw"]


        srgb = srgb.astype(np.float32) / 255.0
        rggb = rggb.astype(np.float32) / 4095.0

        rggb = np.transpose(unprocess.mosaic_rggb(rggb),(1,2,0))

        # raw = raw.astype(np.float32).reshape(srgb.size)
        if self.phase == 'train':
            ##### gt 和 rggb采用同一种增强方法  ####
            r = random.randint(0, 7)
            srgb = aug_img_np(srgb, r)
            rggb = aug_img_np(rggb, r)

        srgb = transforms.ToTensor()(srgb)
        rggb = transforms.ToTensor()(rggb)
        ...
        # if noisy is activated, we also need unprocess the rgb (since we only know the shot and read noise of the Raw).
        if 'raw' in self.in_type or 'lin' in self.in_type:
            data.update({'raw': rggb.clone(), 'linrgb': srgb.clone()})
